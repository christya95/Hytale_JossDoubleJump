...
/**
 * Applies {@link DoubleJumpPhaseLogic} after server input processing so movement state matches the
 * processed tick. Runs {@linkplain Order#AFTER after} {@link PlayerSystems.ProcessPlayerInput}.
 */
public class DoubleJumpSystem extends EntityTickingSystem<EntityStore> {
    ...
    public void tick(
        ...
        if (grounded) {
            DoubleJumpPhaseLogic.onGrounded(dj);
            return;
        }

        DoubleJumpPhaseLogic.tickAirborne(dj, cfg);

        if (ActivationMode.from(cfg) == ActivationMode.JUMP_KEY) {
            Ref<EntityStore> ref = chunk.getReferenceTo(index);
            DoubleJumpPhaseLogic.consumeAirJumpPressIfPending(ref, cmd, dj, cfg);
        }
    }
}
